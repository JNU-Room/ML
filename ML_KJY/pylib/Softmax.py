...
class Softmax:
    ...
    def __init__(self,x_data,y_data):
        ...
        self.b = tf.Variable(tf.random_uniform([len(y_data[0])], -1.0, 1.0))



    def set_whc(self,input_length):
        self.weights.append(tf.Variable(tf.random_uniform([input_length,len(self.y_data[0])], -1.0, 1.0)))
        self.hypothesis = tf.nn.softmax(tf.add(tf.matmul(self.X,-self.weights[-1]),self.b))
    ...
    def training(self, learning_rate=0.04, step=2001, show_training_data=False):
        ...
        for step in range(step):
            ...
            self.W_val.append(self.sess.run(self.weights[-1], feed_dict={self.X: self.x_data}))
            ...
            if show_training_data==True and step % 200 == 0 :
                print(step,'weght = ',self.sess.run(self.weights[-1],feed_dict={self.X: self.x_data, self.Y: self.y_data}),'cost =',self.sess.run(self.cost,feed_dict={self.X:self.x_data,self.Y:self.y_data}))
    ...
    def save_weight(self):
        np.save('weight',self.sess.run(self.W))
        np.save('bias',self.sess.run(self.b))
    ...
    def create_layer(self, X, input_length, output_length):
        ...
        ret = tf.matmul(X,self.weights[-1])
        return ret